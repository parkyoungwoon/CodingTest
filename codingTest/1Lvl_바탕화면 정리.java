...
class Solution {
    public int[] solution(String[] wallpaper) {
        /*
        바탕화면 상태를 나타내는 배열 : wallpaper
        왼쪽 가장 위 0,0
        빈칸은 . 파일이 있는 칸 #
        (가로, 세로)
        |1번 x - 2번 x| + |1번 y - 2번 y| = 드래그한 거리
        */
        //정답 배열 (1번 x, 1번 y, 2번 x, 2번 y)
        int[] answer = new int[4];
        
        List<Integer> listX = new ArrayList();
        List<Integer> listY = new ArrayList();
        //전체 사이즈
        int mapX = wallpaper[0].length();
        int mapY = wallpaper.length;
        int xMin = 50;
        int xMax = 0;
        int yMin = 50;
        int yMax = 0;
        /*
        0.map 에 전체 좌표 넣고, x y 최대값 저장
        1.x 좌표의 최소 값과 최대값 찾기
        2.y 좌표의 최소 값과 최대값 찾기
        3.x 좌표의 최소 값, y 좌표의 최소값, x 좌표 최대값, y 좌표 최대값 조합
        */
        for(int i = 0; i < mapY; i++){
            char[] boardArray = wallpaper[i].toCharArray();
            for(int j = 0; j < mapX; j++){
                //map.put(i+"_"+j,Character.toString(boardArray[j]));
                if(Character.toString(boardArray[j]).equals("#")){
                    listX.add(i);
                    listY.add(j);
                }
            }
